...
- `docs/modernization/defect-ledger.md` (DEF-101–113 open dispositions)
...
- “Improving” complex-column order, text formatting, or diagnostics so outputs no longer substitute for legacy for the same inputs. `E2` user 2026-08-18; `E2`/`E3` — BEH-104; GAP-013.
- Inventing a single repository-wide `(Re,Im)` or `(Im,Re)` convention, or a single text codec, that would change some retained surfaces’ I/O. `E2`/`E3` — BEH-104; GAP-013.
- Promoting provisional tolerances (`1e-6` / `1e-10`) or Python-generated “goldens” as accepted product parity policy without tying them to substitutable I/O. `E1`/`E2`/`E3`/`E4` — INT-006/007; BEH-101/103; oracle via ASSESSMENT §7.
...
- Retained surfaces name kind-8 numerics, array layout/bounds, text codecs, and complex-column order as **I/O contracts to reproduce**, not as assumed C# defaults. `E3` — BEH-101–104; user 2026-08-18.
- Help-vs-code and similar contradictions are disposed toward **observable I/O fidelity** (typically `reproduce-faithfully`) unless an owner explicitly chooses a fix that still preserves agreed swap fixtures. `E2` user 2026-08-18; DEF-101–113.
...
- [ ] **Comparison policy detail:** For each retained surface, is substitutable output judged by **exact bytes**, **normalized text**, and/or **parsed numeric** equality—and which absolute/relative/ULP/residual thresholds replace provisional `1e-6` / `1e-10` where exact match is impossible? Direction: whatever preserves drop-in substitution for real consumers. Numbers/rules still TBD. `E1`/`E2`/`E3`/`E4`/`E5` — BEH-101/103; INT-006; DEF-108; user 2026-08-18.
- [ ] **Measured STOP exit status:** What exit status does bare `STOP` produce on the accepted POC runtime, and must that numeric code be part of the swap contract? Channel/termination *shape* is resolved above; host exit-code bytes remain `E5` until captured. `E3`/`E5` — BEH-105; DEF-109.

## Links

- Related requirements: none yet (behavior catalog BEH-101–105 is the recovered contract source for this slice)
