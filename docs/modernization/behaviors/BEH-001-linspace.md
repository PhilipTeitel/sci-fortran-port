...
**Status:** Recovered; first code slice (SL-001). Requirements drafted (`docs/requirements/REQ-001-linspace.md`, Draft). Ready for `/design-application`.
...
| `num < 0` calls `error("linspace: N<0, abort.")` then `STOP`. | `E3 code-derived` — `src/tools_grids.f90:7`; `src/COMVARS.f90:189-199` | no for first slice — REQ-001 S3 typed domain failure; Fortran text/STOP not parity |
| Both endpoints included and `num < 2` calls `error("linspace: N<2 with both start and end points")`. | `E3 code-derived` — `src/tools_grids.f90:12` | no for first slice — REQ-001 S4; unexecuted string not parity |
| If only start is included, `step = (stop-start)/num` and samples are `start + (i-1)*step` for `i=1..num` (stop excluded). | `E3 code-derived` — `src/tools_grids.f90:16-18` | deferred — REQ-001 non-goal |
| If only stop is included, `step = (stop-start)/num` and samples are `start + i*step` for `i=1..num` (start excluded). | `E3 code-derived` — `src/tools_grids.f90:20-22` | deferred — REQ-001 non-goal |
| If neither endpoint is included, `step = (stop-start)/(num+1)` and samples are `start + i*step` for `i=1..num`. | `E3 code-derived` — `src/tools_grids.f90:24-27` | deferred — REQ-001 non-goal |
| Result array is sized as `array(num)` in the function header before the `num<0` check. | `E3 code-derived` — `src/tools_grids.f90:1-7` | DEF-002 M3 label open; not a managed-API contract (REQ-001 S3) |
| CLI help describes the same job (“evenly spaced numbers over a specified interval”) with defaults `wmin=-5`, `wmax=5`, `L=1024`. | `E3 code-derived` — `numutils/src/linspace.f90:17-25` | out of first slice — BEH-201 |

## 6. Error handling and edge cases

| Case | Legacy behavior | Evidence | Defect decision |
|------|-----------------|----------|-----------------|
| `num < 0` | `error` + `STOP` | `E3` — `src/tools_grids.f90:7` | REQ-001 S3: typed rejection, no sequence. DEF-002 Fortran sizing hazard label still TBD |
| Inclusive endpoints and `num < 2` | `error` + `STOP` | `E3` — `src/tools_grids.f90:12` | REQ-001 S4: typed rejection, no sequence; Fortran string not parity |
| `start == stop`, `num >= 2`, inclusive | Constant sequence at `start` by the formula | `E4 inferred` from formula; not executed | REQ-001 S2 evaluation rule; not an extra FIX |
| Decreasing interval (`start > stop`) | Negative step; still includes both ends | `E4 inferred` from formula; not executed | REQ-001 S2 evaluation rule; not an extra FIX |
...
Superseded by refined scenarios **S1–S5** in `docs/requirements/REQ-001-linspace.md`. Recovery drafts below are retained for provenance only.
...
Scenario: Invalid negative length
  ...
  # Refined as REQ-001 S3: typed domain failure, no sequence; Fortran message/STOP not first-slice parity.
```
...
Closed or deferred by `/refine-feature` into `docs/requirements/REQ-001-linspace.md` (Draft). Remaining items are out of this slice, not unanswered first-slice blockers.
...
- [x] Should optional `istart`/`iend`/`mesh` be part of the managed port in this slice or deferred? **Deferred** — REQ-001 non-goal; later REQ needs fixtures.
- [x] What typed exception/result type and message stability are required for aborting cases? Typed domain failure, no sequence (ADR-002). Concrete type names are design. Fortran strings are not first-slice parity (REQ-001 S3/S4).
- [x] Must `start == stop` and decreasing intervals be specified before implementation, or only FIX-001? Formula is the S2 evaluation rule; FIX-001 remains the only exact-equality parity fixture (ADR-003).
...
- Requirements: `docs/requirements/REQ-001-linspace.md` (S1–S5)
...
*Created: 2026-08-19 | Refined: 2026-08-19 (`/refine-feature` → REQ-001)*
