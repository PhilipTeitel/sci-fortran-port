...
| INT-010 | `/plan-migration` sequenced retained catalog IDs as SL-001–SL-025; first code-production command is `/refine-feature` on BEH-001; catalog-only rows stay document→refine→design→implement. **Superseded 2026-08-19 by ADR-006/007/008:** planned work is three vertical slices (VS-1 `BEH-001`, VS-2 `BEH-003`, VS-3 `BEH-040`); CLI slices are withdrawn; remaining modules are reserve. **2026-08-20:** `/refine-feature` produced `REQ-001`; `/design-application` produced `README.md` and ADR-009/010; `/plan-port-story` produced `VS1-1`, which `/implement-story` then delivered with `FIX-001` parity green; next is `/review-story VS1-1`, then `/verify-parity VS1-1`, then `/plan-project` to open VS-2. | Migration sequencing | E2 documented | `docs/modernization/migration-plan.md`; `docs/requirements/REQ-001-linspace.md`; catalog 2026-08-19; ADR-006/007/008 | high | migration plan / BEH-001 / REQ-001 |
| INT-011 | Owner marked `REQ-001` **`Ready for Design`** on 2026-08-20, accepting scenarios S1–S6 and the recorded `E4`/`E5` decisions (optional flags deferred, S3/S4 specified from the formula but not T1, invalid length as a typed domain failure). This closes the requirements gate (Gate 2) for VS-1. Design acceptance was a separate decision, recorded as INT-012. | First-slice requirements approval | E2 documented | Owner decision 2026-08-20; `docs/requirements/REQ-001-linspace.md` Status; `docs/features/VS1-1-managed-linspace-port.md` §3 | high | REQ-001 / ADR-009 / ADR-010 / VS1-1 |
| INT-012 | Owner accepted **ADR-009** (solution layout, `SciFor.Domain`/`Application`/`Managed`, port `IGenerateLinearSequence`, adapter `Grids.Linspace`, rank-1 zero-based results) and **ADR-010** (`DomainFailureException` base, `LinearSequenceRejection`, `LinearSequenceRejectedException`, stable `Code` values, no process `STOP`) on 2026-08-20, closing Gate 4 and authorizing VS-1 implementation. Story `VS1-1` was then implemented: `FIX-001` parity holds under exact parsed equality, and the layout and typed-failure pattern are now the precedent VS-2 and VS-3 reuse. | First-slice design approval and delivery | E1 verified (tests green) / E2 documented | Owner decision 2026-08-20; ADR-009/010 Status; `docs/features/VS1-1-managed-linspace-port.md` §8; `dotnet test` | high | ADR-009 / ADR-010 / VS1-1 / FIX-001 |
| INT-013 | `/plan-story VS-1` was invoked on 2026-08-20 and **no story was written**, because three conditions the command requires are not met. (a) `VS-1` is a migration-plan slice ID, not a story ID: the slice's story namespace is `VS1-N`, its one story `VS1-1` is `In Progress`, and the architect's preserve rule forbids re-planning or renumbering a row that is not `Not Started`, so there was no backlog row to expand. (b) The configured lane for this repository is `modernizationPort`, whose planning command is `/plan-port-story`; the plain story template carries no `Phase P` or `8b. Parity Plan`, and VS-1 claims `FIX-001` parity. (c) `DEF-005` is undecided, so no new story covering the `BEH-001` happy path can pass its Definition of Ready without inventing the owner's disposition. VS-1 needs no further planning: `S1`–`S6` are all mapped in `VS1-1` §8a, and its only open gates are `Z6`–`Z8`, owned by `/review-story VS1-1` and `/verify-parity VS1-1`. | Workflow sequencing / first-slice planning | E1 verified (VS1-1 re-verified on a clean toolchain 2026-08-20: `dotnet build` 0 warnings / 0 errors, `dotnet test` 34 passed, `parity_BEH_001_P1` passed, `dotnet format --verify-no-changes` clean) / E2 documented (command contract; architect preserve rule) | `/plan-story VS-1` invocation 2026-08-20; `README.md` Backlog Items Epic 1; `docs/features/VS1-1-managed-linspace-port.md` §8a/§12; `docs/modernization/defect-ledger.md` DEF-005; `.cursor/workflow.config.yml` `retainedScope.nextCommand` | high | VS1-1 / DEF-005 / migration plan |
...
*Created: 2026-08-10 | First-slice intent: 2026-08-19 | Whole-library purpose: 2026-08-19 | `/plan-migration`: 2026-08-19 | `REQ-001` marked `Ready for Design` (INT-011): 2026-08-20 | ADR-009/010 accepted and VS1-1 delivered (INT-012): 2026-08-20 | `/plan-story VS-1` declined with no story written (INT-013): 2026-08-20*
