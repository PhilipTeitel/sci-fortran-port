...
| INT-011 | Owner marked `REQ-001` **`Ready for Design`** on 2026-08-20, accepting scenarios S1–S6 and the recorded `E4`/`E5` decisions (optional flags deferred, S3/S4 specified from the formula but not T1, invalid length as a typed domain failure). This closes the requirements gate for VS-1. Acceptance of the **design** is separate and still open: ADR-009 and ADR-010 remain `Proposed`, so story `VS1-1` is planned but not Ready, and no C# may be written yet. | First-slice requirements approval | E2 documented | Owner decision 2026-08-20; `docs/requirements/REQ-001-linspace.md` Status; `docs/features/VS1-1-managed-linspace-port.md` §3 | high | REQ-001 / ADR-009 / ADR-010 / VS1-1 |
